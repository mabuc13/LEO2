...
class Analyzer:
    ...
    def historgram(self):
        ...
        if self.showPlot:
            ...
            plt.savefig('flanks_histogram.png')  # save the figure to file
            plt.close()
    def period_histogram(self):
        ...
        # Plot the histogram.
        plt.hist(period_list, bins=100, alpha=0.6, color='r')
        title = "Histogram with period intervals: mu = %f  std = %f \n units are %s. The frequency is: %i [Hz]" % (self.period_mean, self.period_std, self.timeunit, self.freq)
        plt.title(title)
        plt.ylabel('Number of occurences')
        plt.xlabel(self.timeunit)
        if self.showPlot:
            ...
            plt.savefig('period_histogram.png')  # save the figure to file
            plt.close()
    #plt.show()
    #plt.plot(timeRefined, ampRefined, label = "Amp after sorting", linestyle="-",marker=".")
    #plt.show()
